...
public sealed class DefenseGameSession
{
    ...
    public DefenseGameSession(DungeonState initialDungeon)
        : this(PlayerDungeonState.FromSingleFloor(initialDungeon, ResolveProfileId(initialDungeon)))
    {
    }

    public DefenseGameSession(PlayerDungeonState initialDungeon)
    {
        DungeonEditor = new PlayerDungeonEditorSession(initialDungeon);
        EditorCommands = new DefenseEditCommandService(DungeonEditor);
    ...
    private DungeonStaticFileService CreateStaticFileService()
    {
        ...
        return new DungeonStaticFileService(Editor, boardProfileId: floor.BoardProfileId);
    }
...
public static class DefenseSliceScenario
{
    ...
    public static DefenseGameSession CreateMultiFloorSession(params string[] boardProfileIds)
    {
        ...
        return new DefenseGameSession(dungeon);
    }
    ...
    public static void ConfigureBoardDemonstration(DefenseGameSession session, string boardProfileId, string floorId)
    {
        ...
        if (boardProfileId == DungeonBoardProfiles.PillaredCryptId)
        {
            ...
            Apply(session.EditorCommands.Execute(new PlaceGuardCommand("G-PIL-SW", DefenseSliceBuildCatalog.SkeletonWarrior.Id, 4, 4, floorId)));
            Apply(session.EditorCommands.Execute(new PlaceGuardCommand("G-PIL-SA", DefenseSliceBuildCatalog.SkeletonArcher.Id, 9, 4, floorId)));
        ...
        if (boardProfileId == DungeonBoardProfiles.ManaFaultId)
        {
            ...
            Apply(session.EditorCommands.Execute(new PlaceGuardCommand("G-MAN-SW", DefenseSliceBuildCatalog.SkeletonWarrior.Id, 5, 3, floorId)));
            Apply(session.EditorCommands.Execute(new PlaceGuardCommand("G-MAN-SA", DefenseSliceBuildCatalog.SkeletonArcher.Id, 9, 2, floorId)));
    ...
    public static void ConfigureSuccessfulDefense(DefenseGameSession session, string floorId)
    {
        ...
        Apply(session.EditorCommands.Execute(new PlaceGuardCommand("G-SW", DefenseSliceBuildCatalog.SkeletonWarrior.Id, 5, 2, floorId)));
        Apply(session.EditorCommands.Execute(new PlaceGuardCommand("G-SA", DefenseSliceBuildCatalog.SkeletonArcher.Id, 8, 3, floorId)));
